...
import Get_Summary
from torchreid.reid.data import *
from torchreid.reid.models import *
from torchreid.reid.optim import *
...
def main():
    ...
    if cfg.model.resume1 and check_isfile(cfg.model.resume1):
        ...
        dir_split = (cfg.model.resume1.split('/'))[:-2] 

        delim = '/'

        dir_path = delim.join( folder for folder in dir_split)
        cfg.data.save_dir = dir_path
        #print('Save dir',cfg.model.resume1)
    else:
        cfg.data.save_dir = cfg.data.save_dir+'/'+time.strftime('%Y_%m_%d_%H_%M_%S')
    '''
    log_name = 'TestLog_'+time.strftime('%Y_%m_%d_%H_%M_%S')+'.log' if cfg.test.evaluate else 'TrainLog_'+time.strftime('%Y_%m_%d_%H_%M_%S')+'.log'
    #log_name += time.strftime('_%Y_%m_%d_%H_%M_%S')
    ...
    # use the file path of chk pt to create a .pth file from the yaml file
    Savemodel.savemodelfile('ModelFolder','89EModel',model1,optimizer1,cfg.model.resume1)
'''
